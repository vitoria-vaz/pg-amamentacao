...
# 1. Mapeamento do diretório gerado no pré-processamento via K-Fold
...
# 0 = 'desmame_precoce_<_6m', 1 = 'sucesso_ame_6m+'
le = LabelEncoder()

# 3. Verificação do diretório
if not os.path.exists(base_dir):
    print(f"Diretório não encontrado: {base_dir}. Certifique-se de ter rodado o script de divisão K-Fold antes.")
else:
    # Capturar todas as pastas (fold_1, fold_2, ..., fold_5) ordenadas alfabeticamente
    ...
    for fold in folds:
        ...
        # Carregar os arquivos CSV separados deste Fold
        ...
        # Alinha as colunas de treino e teste (garante que ambas tenham as mesmas features após get_dummies)
...
df_resultados = df_resultados.sort_values(by=['Quantidade Atributos', 'Fold', 'Algoritmo'])
df_matrizes = df_matrizes.sort_values(by=['Quantidade Atributos', 'Fold', 'Algoritmo'])

# Caminho do arquivo de saída (Nome ajustado para os 30 atributos)
caminho_tabela = os.path.join(output_dir, 'tabela_comparativa_30_atributos.xlsx')
